...
import { Button, Card, CardActions, CardContent, FormControl, Input, InputLabel, MenuItem, Select, SelectChangeEvent } from '@mui/material';
...
export function EditCharacterForm(props: EditCharacterFormProps): JSX.Element {
  ...
  return (
    <Card sx={{ width: 300, height: 480 }}>
      <CardContent>
        <form onSubmit={handleEditSubmit} className={classNames(cls.editcharacterform, {}, [className])}>
      ...
      <CardActions>
        {
          editting && (
            <Fragment>
              <Button type="button" size="large" color="warning" onClick={handleEditCancel}>Cancel</Button>
              <Button type="submit" size="large" color="warning" onClick={handleEditSubmit}>Save</Button>
            </Fragment>
          )
        }
        { !editting && (
            <Button color="warning" type="button" size="large" onClick={handleEditStart}>EDIT</Button>
          )
        }
      </CardActions>
    </Card>
